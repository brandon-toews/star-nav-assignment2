...
//Class used to draw suggested in Dijkstra pathfinding that inherits from a base class for drawing star connections
public class SuggestedRoute : GenerateStarConnections {

    //Set bool lerping to false to start scale lerp on meshes that draw the connections in the path
    public bool lerping = false;

    //Create mesh for instance of this class set the scale and mesh sides
    private void Awake() {
    ...
    void Generate() {
        ...
        //keep lerping scale of drawn connections
        if (!lerping) StartCoroutine(Lerp());

        //draw connections in the suggested path
        DrawStarConnections();
    }

    //to lerp scale of meshes
    private IEnumerator Lerp() {

        //Set lerping to true so that lerping starts
        lerping = true;

        //Set time to zero
        float time = 0f;

        //Creating scale values without stopping
        while (lerping) {

            //because this is continuous action speeds up unless reset back to zero after every sec
            if (time > 1f) time = 0f;

            //Generate scale values by access Lerp function library and feeding linear ease and pingpong parameters
            scale = LerpFunctions.Lerping(0.2f, 1.0f, LerpFunctions.LerpPerc(1, time, true));
    ...
    public override int CountStarConnections() {
        ...
        //If there are no stars selected for path finding return zero
        if (starsToConnect.Count == 0) return 0;
        //else take the amount of stars in list and minus one
        else return starsToConnect.Count - 1; 
    }

    //override to draw connections for suggested path
    public override void DrawStarConnections() {
        ...
        //Loop thru each star in list of suggected path
        foreach (Star star in starsToConnect) {
            ...
            foreach (KeyValuePair<Star, float> conStar in star.connectedStars) {
                ...
                //if the selected connecting star is found in the visitedstars list and the starsToConnect list then don't draw the connection
                if (!visitedStars.Contains(conStar.Key) && starsToConnect.Contains(conStar.Key)) {
